...
func doMap(
	...
	for i := 0; i < nReduce; i++ {
		...
		defer f.Close()
		files = append(files, encoder)
	}

	s := string(bytes)
	var mappedKVs = mapF(inFile, s)
	for i := 0; i < len(mappedKVs); i++ {
		hash := ihash(mappedKVs[i].Key) % nReduce
		kv := KeyValue{mappedKVs[i].Key, mappedKVs[i].Value}
		files[hash].Encode(&kv)
	}
}
